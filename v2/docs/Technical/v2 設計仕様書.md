...
### Bluesky 投稿仕様（バニラ状態 - bluesky_core.py）
...
| `NICONICO_USER_NAME` | - | ニコニコユーザー名（ニコニコプラグイン用、自動取得・自動保存対応） |
...
├── bluesky_core.py（Bluesky API コア）
...
- **通知レイヤー（main_v2.py / gui_v2.py → bluesky_core.py + bluesky_plugin.py）**
  - DB の未投稿動画を取得：`posted_to_bluesky = 0` の動画を抽出
  - 投稿形式生成：
    - バニラ状態：bluesky_core.py で source（YouTube/Niconico）に応じた一定のテンプレート生成
    （シンプルな形式、テキストのみ）
  ...
  - 無限ループで以下を繰り返す:
    ...
    6. ニコニコプラグイン・YouTube Live プラグインが有効な場合は併行して監視：
       - **ニコニコプラグイン有効時**：
         ...
         - **ユーザー名自動取得**（v2.2.0以降）：
           - 優先順位: RSS <dc:creator> > 静画API > ユーザーページ og:title > 環境変数 NICONICO_USER_NAME > ユーザーID
           - 取得されたユーザー名は `settings.env` に自動保存（2回目以降は API 呼び出し不要）
           - プラグインインスタンス単位でメモリキャッシング
