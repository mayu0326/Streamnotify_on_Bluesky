...
- 実装計画書
- 修正完了報告書
など、設計・変更履歴に関するドキュメントはすべて `v2/docs/local/` に配置
...
2. **ドキュメント作成** → `v2/docs/local/error_investigation_*.md`
   - エラー内容
   - 原因分析
   - 解決策（短期/中期/長期）
...
1. **計画書作成** → `v2/docs/local/*_plan.md`
   - 実装内容
   - 対象ファイル
   - 期待するアウトプット
...
4. **完了報告書作成** → `v2/docs/local/IMPLEMENTATION_COMPLETE_*.md`
   - 実装内容
   - テスト結果
   - 改善指標
...
- [ ] エラー調査のときは `v2/docs/local/` にドキュメント作成
- [ ] テストスクリプト作成のときは `test_scripts/` に配置
- [ ] 本番コード修正は `v2/` 以下で実施
- [ ] 修正完了後は `v2/docs/local/` に報告書を作成
...
1. YouTube Live 判定ロジック整理
   - 計画書: `v2/docs/local/youtube_live_classification_plan.md`

2. Niconico ID エラーの修正（完了）
   - 調査: `v2/docs/local/error_investigation_sm45414087.md`
   - 実装: `v2/docs/local/fix_youtube_video_id_validation.md`
   - 完了: `v2/docs/local/IMPLEMENTATION_COMPLETE_VIDEO_ID_VALIDATION.md`
