...
### 🆕 2.1 画像自動リサイズ機能（v2.1.0 - 2025-12-17 実装）
...
| 最大サイズ | ✅ 1MB/画像 |
| **画像自動リサイズ機能** | **✅ v2.1.0 で実装済み（2025-12-17）** |
...
## 8. テンプレート機能の統合（2025-12-18 実装完了）

### 8.1 概要

Bluesky プラグインに実装されたテンプレート機能が、YouTube・ニコニコの新着動画投稿に統合されました。
...
【画像処理】embed 構築
  ↓
【テンプレートレンダリング】← NEW (2025-12-18)
  ├─ source = "youtube" → youtube_new_video テンプレート
  ├─ source = "niconico" → nico_new_video テンプレート
  └─ video["text_override"] に結果を格納
  ↓
post_video_minimal()
  ├─ text_override 存在？ → YES: テンプレート本文優先
  └─ NO: 従来固定フォーマット使用
  ↓
【投稿実行】
  ├─ Facet 構築（URL リンク化）
...
- **プラグイン未導入**: `text_override` 未設定 → 従来フォーマット使用
- **テンプレートファイルなし**: レンダリング失敗 → 従来フォーマット使用
- **必須キー不足**: レンダリング失敗 → 従来フォーマット使用

**既存ユーザーに影響なし** - テンプレートが設定されていなければ、今まで通りの投稿が行われます。

### 8.5 詳細ドキュメント

テンプレート機能の詳細な実装については、[TEMPLATE_INTEGRATION_v2.md](TEMPLATE_INTEGRATION_v2.md) の「セクション 12. テンプレート機能の投稿統合」を参照してください。
...
- [x] **テンプレート機能の投稿統合** - YouTube/ニコニコ対応（2025-12-18 実装完了）
...
**このドキュメントは v2.1.0（2025-12-17 テンプレート統合）・v2.1.1（2025-12-18 テンプレート投稿統合）実装済みの統合技術資料です。
