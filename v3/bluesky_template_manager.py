...
【ステータス】⏳ 将来実装予定（デモ版/プロトタイプ）
このモジュールは GUI テンプレート編集機能の実装時に統合される予定です。
現在は実装は完了していますが、GUI に組み込まれていません。
...
利用予定:
-------
- v3.x で「テンプレート設定」タブを GUI に追加時に統合予定
- `gui_v3.py` の TemplateEditorFrame から `open_template_editor_from_gui()` を呼び出し予定

注意:
-----
Vanilla 環境では Bluesky プラグイン無効のため、
このモジュールも読み込まれません。
