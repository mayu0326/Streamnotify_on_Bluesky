...
for i in range(0, len(video_ids), batch_size):
    ...
    print(f"バッチ {i//batch_size + 1}: {len(batch)} 件を処理 ({i+1}-{min(i+batch_size, len(video_ids))}/{len(video_ids)})")
...
if cache_file.exists():
    ...
    print(f"\n次回以降、このキャッシュが利用されます！")
    print(f"API コスト削減: {len(api_plugin.video_detail_cache)} 件 × 1 ユニット = {len(api_plugin.video_detail_cache)} ユニット節約可能 ✅")
