...
class BlueskyImagePlugin(NotificationPlugin):
    ...
    def post_video(self, video: dict) -> bool:
        ...
        """
        # ★ メソッド入り口で入力値をチェック
        post_logger.info(f"📥 【post_video() 入力値】 classification_type={video.get('classification_type')}, content_type={video.get('content_type')}, live_status={video.get('live_status')}, event_type={video.get('event_type')}")
        
        # ★ classification_type が None の場合、content_type から自動判定
        if not video.get('classification_type') and video.get('content_type'):
            if video.get('content_type') == 'live':
                video['classification_type'] = 'live'
                post_logger.info(f"✅ 【自動判定】 content_type='live' → classification_type='live' に設定")
            elif video.get('content_type') == 'archive':
                video['classification_type'] = 'archive'
                post_logger.info(f"✅ 【自動判定】 content_type='archive' → classification_type='archive' に設定")
        ...
        # ★ コピー直後にフィールド値をチェック
        post_logger.info(f"📋 【dict(video)直後の入力値チェック】 classification_type={video.get('classification_type')}, content_type={video.get('content_type')}, live_status={video.get('live_status')}")
        ...
        if source == "youtube":
            ...
            if classification_type == "live":
                # ライブ開始テンプレート
                post_logger.info(f"✅ classification_type='live' 条件に合致。youtube_online を試みます")
                ...
                if rendered:
                    ...
                    post_logger.info(f"⚠️ youtube_online テンプレート失敗。デフォルトフォーマットを使用します")
            elif classification_type == "schedule" or live_status == "upcoming":
                # ★ 放送枠予約テンプレート: 拡張時刻を計算
                post_logger.info(f"✅ classification_type='schedule' または live_status='upcoming' 条件に合致。youtube_schedule を試みます")
            ...
            elif classification_type == "completed" or live_status == "completed":
                # ★ 放送終了テンプレート: ライブ配信が終了した時の通知
                post_logger.info(f"✅ classification_type='completed' または live_status='completed' 条件に合致。youtube_offline を試みます")
                rendered = self.render_template_with_utils("youtube_offline", video)
                if rendered:
                    video["text_override"] = rendered
                    post_logger.info(f"✅ テンプレートを使用して本文を生成しました: youtube_offline (classification_type='completed' or live_status='completed')")
                else:
                    # youtube_offline テンプレート未設定時は新着動画テンプレートにフォールバック
                    post_logger.debug(f"ℹ️ youtube_offline テンプレート未使用。youtube_new_video にフォールバック")
                    rendered = self.render_template_with_utils("youtube_new_video", video)
                    if rendered:
                        video["text_override"] = rendered
                        post_logger.info(f"✅ テンプレートを使用して本文を生成しました: youtube_new_video (フォールバック)")
                ...
                post_logger.info(f"✅ classification_type='archive' または content_type='archive' 条件に合致。youtube_archive を試みます")
                ...
                # 通常動画用テンプレート（デフォルト）
                post_logger.info(f"⚠️ 条件不一致 → デフォルトフォーマット処理。classification_type={classification_type}")
                rendered = self.render_template_with_utils("youtube_new_video", video)
                if rendered:
                    video["text_override"] = rendered
