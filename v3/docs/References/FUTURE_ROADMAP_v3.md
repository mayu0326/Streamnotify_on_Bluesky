...
|:--|:--|:--|:--|
| **YouTube Live** | ✅ 実装済み | 2025-12-18 | youtube_online テンプレート |
...
- **フル機能版 GUI**（Tkinter）:
  - v2 にある全ての機能に加えて、以下の新機能を実装
    - **✅ 重複投稿防止オプション**（実装済み）
      ...
      - YouTube 優先度ベースの重複排除（新動画 > アーカイブ > 通常動画）
 ...
 ### v3 での実装（v3.2.0で完了）
  - **✅ 動画フィルタ・検索機能**（実装済み）
      - タイトル検索フィルタ（リアルタイム検索）
      - 配信元別フィルタ（YouTube/Niconico - 大文字小文字対応）
      - 投稿状態フィルタ（Posted/Unposted）
  - ✅ **コンテンツタイプフィルタ**（v3.2.0で実装済み）
      - Video（🎬 動画）、Archive（📹 アーカイブ）、Live（🔴 配信）
      - 大量動画時に対象動画を素早く見つけられる
      - フィルタ条件の保存機能（v3.x予定）
  - **✅ テンプレートシステムの完全実装**（v3.2.0で実装済み）
      - Jinja2 テンプレートレンダリング
      - サービス・イベント別の自動選択（classification_type による判定）
      - テンプレート変数の検証と必須キー管理
      - フォールバック機能（テンプレート未設定時は自動フォールバック）
      - 複数フィルター対応（日付フォーマット、ハッシュタグなど）
  - **✅ RSS手動更新トリガー（v3.2.0で実装完了）**
      - GUI に「今すぐ更新」ボタン追加
      - テスト・デバッグ時に便利、ポーリング待機不要
  - **✅ GUI統計表示（v3.2.0で実装完了）**
      - 全体統計：監視動画数、投稿済み数、投稿待機数
      - 配信元別統計：YouTube/ニコニコ別の投稿済み件数
      - 過去7日間の日別投稿状況表示
      - 投稿成功率・エラーログなどの可視化
      - メリット: トラブルシューティング時に便利、運用監視向上
  - **✅ バックアップ・復元機能** (v3.2.0で実装完了)
      - ZIP形式でDB・テンプレート・設定をエクスポート/インポート
      - API キー・パスワード除外オプション（YouTube/Niconico/Twitch ID対応）
...
- [✅] **テンプレート条件分岐の拡張**: category別など複雑な条件分岐対応（v3.2.0で実装完了）
...
### Phase 3.1（v3.2.0 で実装予定）
- [ ] **フィルタ条件の保存機能**: GUI で設定したフィルタをプロファイル保存
  - フィルタプロファイルの作成・管理
  - 複数プロファイルの保存・切り替え機能
  - デフォルトプロファイルの自動適用

### Phase 3.2（v3.3.0 で実装予定）
