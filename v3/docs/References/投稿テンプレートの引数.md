...
- `{{ platform }}` … プラットフォーム（例: "YouTube"）**【新規】**
...
- `{{ channel_name }}` … チャンネル名
- `{{ published_at }}` … 配信開始日時（ISO8601形式、例: 2025-12-18T10:30:00）
- `{{ live_status }}` … ライブ配信ステータス

---

### 【YouTube放送枠予約通知】
（yt_schedule_template.txt、`放送枠予約`）

- `{{ title }}` … 放送予定のタイトル
- `{{ video_url }}` … 放送枠のURL（例: https://www.youtube.com/watch?v=xxxx）
- `{{ channel_name }}` … チャンネル名
- `{{ published_at }}` … 放送予定日時（ISO8601形式、例: 2025-12-18T10:30:00）
- `{{ live_status }}` … ライブ配信ステータス（`"upcoming"` = 放送前）
- `{{ platform }}` … プラットフォーム（例: "YouTube"）
...
### 【YouTubeアーカイブ通知】
（yt_archive_template.txt、`アーカイブ動画`）
- `{{ title }}` … アーカイブタイトル
- `{{ video_url }}` … アーカイブURL（例: https://www.youtube.com/watch?v=xxxx）
- `{{ channel_name }}` … チャンネル名
- `{{ published_at }}` … 配信日時（ISO8601形式、例: 2025-12-18T10:30:00）
...
- `{{ video_id }}` … 動画ID
- `{{ video_url }}` … 動画URL
- `{{ channel_name }}` … ユーザー名またはチャンネル名（`settings.env` の `NICONICO_USER_NAME` から取得）
- `{{ published_at }}` … 公開日時（ISO8601形式、例: 2025-12-18T10:30:00）
...
3. アプリケーションを再起動
4. `v3/logs/` フォルダのログファイルを確認

---

> **詳細情報**: 完全なテンプレートシステムの説明は [v3/docs/Technical/TEMPLATE_SYSTEM.md](../Technical/TEMPLATE_SYSTEM.md) をご参照ください
