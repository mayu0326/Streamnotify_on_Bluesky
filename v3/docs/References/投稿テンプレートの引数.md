...
### 【YouTube配信枠設置通知】
（yt_schedule_template.txt、`配信枠設置`）
...
## 将来実装予定: Twitch テンプレート変数

> ⏳ **ステータス**: Twitch プラグインはまだ実装していません。以下は将来実装時の変数リファレンスです。
...
│   │   ├── yt_schedule_template.txt       ← 配信枠設置用（将来）
│   │   ├── yt_archive_template.txt        ← アーカイブ通知用
│   │   ├── yt_online_template.txt         ← 配信開始用（将来）
│   │   └── yt_offline_template.txt        ← 配信終了用（将来）
│   ├── niconico/
│   │   └── nico_new_video_template.txt    ← 新着動画投稿用
│   └── default/（フォールバック用）
│       ├── default_template.txt           ← デフォルト汎用テンプレート
