...
- [投稿テンプレート変数リファレンス](../References/投稿テンプレートの引数.md) を参照してください。
...
1. パスが正しいか確認
   ```
   ✅ 正しい: v3/templates/youtube/yt_new_video_template.txt
   ❌ 間違い: templates\YouTube\yt_new_video_template.txt (バックスラッシュ、大文字)
   ```

2. ファイルが存在するか確認
   - Windows エクスプローラーで `v3/templates/` を開く
...
| GUI テンプレートエディタ | ⏳ 将来実装 | - |
| リアルタイムプレビュー | ⏳ 将来実装 | v3.x |
| テンプレート変数自動補完 | ⏳ 将来実装 | v3.x |
| テンプレート管理機能 | ✅ 実装済み | - |
