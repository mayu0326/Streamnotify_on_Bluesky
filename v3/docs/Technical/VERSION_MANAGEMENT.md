...
```python
__version__ = "3.1.0"                    # セマンティックバージョニング
__release_date__ = "2025-12-18"         # リリース日
...
# 戻り値例: "v3.1.0 (2025-12-18) [development]"
```
...
#     "formatted": "v3.1.0 (2025-12-18) [development]"
# }
```
...
| ステータス | 説明 | 出力例 |
|-----------|------|-------|
| `development` | 開発中 | `v3.1.0 (2025-12-18) [development]` |
| `alpha` | アルファ版 | `v3.1.0 (2025-12-18) [alpha]` |
| `beta` | ベータ版 | `v3.1.0 (2025-12-18) [beta]` |
| `stable` | 安定版 | `v3.1.0 (2025-12-18)` |
...
|-----------|:--------:|------|:----------:|
| **logging_plugin** | 2.0.0 | 複数ロガー、日次ローテーション、環境変数制御 | ✅ v3.3.0 |
| **bluesky_plugin** | 1.1.0 | Bluesky 画像添付、投稿テンプレート拡張 | ✅ v3.3.0 |
| **youtube_api_plugin** | 0.2.0 | YouTube チャンネルID 解決、動画詳細取得、API クォータ管理 | ✅ v3.3.0 |
| **youtube_live_plugin (live_module)** | 0.2.0 | YouTube Live/Archive/Schedule 管理、状態遷移検知、自動投稿 | ✅ v3.3.0 |
| **niconico_plugin** | 0.4.0 | ニコニコ動画 RSS 取得、ユーザー名自動取得・自動保存、リトライ・タイムアウト対応 | ✅ v3.3.0 |
...
出力例:
```
✅ ロギング設定拡張プラグイン v2.0.0
✅ Bluesky 機能拡張プラグイン v1.1.0
✅ YouTubeAPI 連携プラグイン v0.2.0
✅ YouTube Live 管理モジュール v0.2.0
✅ ニコニコ動画 RSS取得プラグイン v0.4.0
```
...
__version__ = "3.2.0"          # 3.1.0 → 3.2.0
__release_date__ = "2026-01-03"  # 更新日
...
- ✅ アプリケーションバージョン機能実装完了
- ✅ モジュール独立バージョン管理実装完了
- ✅ プラグイン独立バージョン管理実装完了（YouTube Live live_module を含む）
...
```bash
# Git タグ作成
git tag -a v3.3.0 -m "v3.3.0 release"
...
例えば、アプリケーション v3.1.0 には、\
- logging_plugin v2.0.0
- bluesky_plugin v1.1.0
- youtube_api_plugin v0.2.0
- youtube_live_plugin v0.2.0
- niconico_plugin v0.2.0 が含まれています.
...
### YouTube Live 機能の完全統合

**v3.2.0 → v3.3.0**
- YouTube Live の判定・投稿機能が `youtube_live_plugin.py` から `plugins/youtube/live_module.py` に統合
- `LiveModule` クラスで Schedule/Live/Completed/Archive を一元管理
- DB スキーマ (content_type, live_status) をそのまま再利用
- 状態遷移検知と自動投稿の責務を明確化

### プラグイン構成の改善

**v3.2.0 → v3.3.0**
- YouTube 関連機能を `plugins/youtube/` パッケージで統合
  - `youtube_api_plugin.py`: チャンネルID解決、動画詳細取得
  - `live_module.py`: YouTube Live 状態管理、自動投稿
- プラグイン間の依存関係を明確化
- テンプレートシステムの v3.2.0 機能を継続サポート

---

**最終更新**: 2026-01-03 (v3.3.0)
